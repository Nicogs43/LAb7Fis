...
public class Controller {
	...
	@FXML
	private Button registraVenditaB, cancellaVenditaB, registraPagamentoB, cancellaPagamentoB, registraRifornimentoB,
			cancellaRifornimentoB;
	...
	@FXML
	private Label messRifornimento, messVendita, messPagamento, messAggiungiPersona, messRimuoviPersona,
			disponibilitaLabel;
	...
	public void rimuoviPersona() {
		...
		if (!personale.removePersona(pers)) {
			...
			Utils.showMessage(messRimuoviPersona, "red",
					"Impossibile rimuovere " + pers.getNome() + " in quanto indebitato");
			return;
		}
	...
	public void registraVendita() {
		...
		if (!Utils.checkPersonToSell(nomePers, cliente, messVendita, nomeVisitatoreTF, venditaPersonaleCB)
				|| !Utils.checkNullCialda(tipoCialda, messVendita)
				|| !Utils.checkDisponibilitaCialde(quantita, tipoCialda, magazzino, messVendita, quantitaCialdeTF))
			return;
		...
		Utils.showMessage(messVendita, "green",
				"Vendita di " + quantita + " " + tipoCialda + " a " + nomePers + " avvenuta");
	...
	public void registraPagamento() {
		...
		cassa.riceviPagamento(euro);
		Utils.showMessage(messPagamento, "green",
				"Sanamento del debito di " + pers.getNome() + " di " + euro + " avvenuto");
	...
	public void registraRifornimento() {
		...
		if (!cassa.effettuaPagamento(new Euro(0, 50 * numeroScatole * 40))) {
			Utils.showMessage(messRifornimento, "red",
					"Inserisci una cifra positiva non superiore alla disponibilità in cassa");
			return;
		}
		// effettua rifornimento
		magazzino.aggiungiRifornimento(numeroScatole, tipoCialda);
		Utils.showMessage(messRifornimento, "green",
				"Rifornimento di " + numeroScatole + " scatole di " + tipoCialda + " avvenuto");
	...
	private void updateDebitiTab() {
		...
		for (Persona pers : personale.getIndebitati()) {
			if (value)
				debitiLeft.getChildren().add(Utils.createDebitiHBox(pers));
			else
				debitiRight.getChildren().add(Utils.createDebitiHBox(pers));
			value ^= true;
		}
	}
